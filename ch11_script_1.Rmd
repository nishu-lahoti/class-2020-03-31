...
# Avg poverty across country

avg_poverty_rate <- county %>%
  select(poverty) %>%
  summarize(avg_rate = mean(poverty))

# Avg poverty in Stark County

avg_poverty_stark <- county %>%
  filter(name == "Stark", state == "OH") %>%
  select(poverty) %>%
  summarize(avg_rate = mean(poverty))

# Avg without hs diploma

avg_less_than_hs <- county %>%
  select(less_than_hs) %>%
  summarize(avg_less_hs = mean(less_than_hs))

# State county highest percentage without hs diploma

county_without_hs <- county %>%
  select(region, state, name, less_than_hs) %>%
...
corr_coeff <- county %>%
  summarize(corr = cor(poverty, less_than_hs))
...
ggplot(county, aes(poverty, less_than_hs, color = region)) + 
  ...
  labs(title = "Correlating Poverty with Education",
       x = "Poverty",
       y = "Percentage without HS Degree",
       color = "Region")
